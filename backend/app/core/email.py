...
async def send_temp_password_email(to_email: str, name: str, temp_password: str, company_name: str) -> None:
    resend.Emails.send({
        "from": settings.FROM_EMAIL,
        "to": [to_email],
        "subject": f"Bienvenido a {company_name} - Warehouse DT",
        "html": f"""
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px; background: #ffffff;">
            <table cellpadding="0" cellspacing="0" border="0" style="margin-bottom: 28px;">
                <tr>
            ...
            <h1 style="font-size: 22px; font-weight: 600; color: #2C2C2A; margin: 0 0 12px 0;">Bienvenido, {name}</h1>
            <p style="font-size: 15px; color: #5F5E5A; margin: 0 0 20px 0; line-height: 1.5;">
                Se ha creado una cuenta para ti en <strong>{company_name}</strong>.<br>
                Usa las siguientes credenciales para acceder a la aplicación móvil:
            </p>
            <table cellpadding="0" cellspacing="0" border="0" width="100%" style="background: #F8F6F0; border-radius: 8px; margin-bottom: 24px;">
                <tr>
                    <td style="padding: 16px 20px;">
                        <p style="margin: 0 0 8px 0; font-size: 12px; color: #888780; text-transform: uppercase; letter-spacing: 0.04em;">Email</p>
                        <p style="margin: 0 0 16px 0; font-size: 15px; font-weight: 600; color: #2C2C2A;">{to_email}</p>
                        <p style="margin: 0 0 8px 0; font-size: 12px; color: #888780; text-transform: uppercase; letter-spacing: 0.04em;">Contraseña temporal</p>
                        <p style="margin: 0; font-size: 18px; font-weight: 700; color: #185FA5; letter-spacing: 0.08em; font-family: monospace;">{temp_password}</p>
                    </td>
                </tr>
            </table>
            <table cellpadding="0" cellspacing="0" border="0" style="background: #FFF8E1; border-radius: 8px; margin-bottom: 24px;" width="100%">
                <tr>
                    <td style="padding: 12px 16px;">
                        <p style="margin: 0; font-size: 13px; color: #856404; line-height: 1.5;">
                            Al iniciar sesión por primera vez el sistema te pedirá que establezcas una contraseña propia.
                        </p>
                    </td>
                </tr>
            </table>
